...
use App\Jobs\CreatePlotsFromExcel;
use App\Models\Clients\Client;
use Excel, Gate;

class PlotsExcelsController extends DashboardController
{
    /**
     * @var protected
     */
    ...
    public function store(ExcelRequest $request)
    {
        ...
        Excel::load(request()->file('upload_excel'), function ($reader) use ($request) {
            $reader->each(function($row) use ($request) {
                dispatch(new CreatePlotsFromExcel($row->toArray(), $request->except('upload_excel')))->delay(lcg_value());
            });
        });
