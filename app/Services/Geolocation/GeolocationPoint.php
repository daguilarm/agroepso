 ...
 *
 * GeolocationPoint (conversion between geographic points) Copyright (C) 2011 Hans Duedal (hd@onlinecity.dk)
 *
...
class GeolocationPoint
{
    ...
    public function convertLatLngToUtm($latitude, $longitude)
    {
        ...
        $UTMNorthing = number_format($UTMNorthing, 2, '.', '');
        $UTMEasting = number_format($UTMEasting, 2, '.', '');

        return [$UTMEasting, $UTMNorthing, $this->srs($ZoneNumber)];
    ...
    /**
     * Get the srs from UTM
     *
     * @param string
     */
    public function srs($zone)
    {
        switch ($zone) {
            case '27':
                return 'EPSG:32627';
